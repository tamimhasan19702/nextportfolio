...
const Navbar = ({ navbar }: NavbarProps) => {
  ...
  const sequences = navbar?.logoSequences?.map((s) => s.label).filter(Boolean) || [
    "{{}}",
    logoText,
    "{{TAREQ MONOWER}}",
  ];
  ...
  useEffect(() => {
    tabs.forEach(({ url }) => router.prefetch(url));
    links.forEach(({ url, target }) => {
      if (target !== "new" && !url.startsWith("#")) router.prefetch(url);
    });
  }, [links, router]);
    ...
    <>
      ...
      <nav className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-1.5 sm:gap-2 rounded-full border border-zinc-200 bg-white/85 backdrop-blur p-1 sm:p-1.5 shadow-lg max-w-[calc(100vw-2rem)]">
        {tabs.map(({ url, label, Icon }) => {
          const active =
            pathName === url || (url === "/portfolio" && pathName.startsWith("/portfolio"));
          return (
            <Link
              key={url}
              href={url}
              ...
              className={`flex items-center gap-2 rounded-full p-2 sm:px-4 sm:py-2 transition-all duration-300 ${
                active ? "bg-black text-white" : "text-zinc-600 hover:bg-zinc-100 hover:text-black"
              }`}
            >
              <Icon size={18} strokeWidth={active ? 2.5 : 2} />
              <span className="hidden sm:inline text-sm font-bold whitespace-nowrap">{label}</span>
            </Link>
          );
        })}
          ...
          <div className="relative">
                      ...
                      >
                        {isHash ? (
                          <motion.button
                            ...
                            onClick={() => {
                              setOpen(false);
                              document.getElementById(link.url.slice(1))?.scrollIntoView({
                                behavior: "smooth",
                              });
                            }}
                            title={link.label}
            ...
            <button
              ...
              className={`p-2 rounded-full transition-all duration-300 ${
                open ? "bg-black text-white" : "text-zinc-600 hover:bg-zinc-100 hover:text-black"
              }`}
            >
